...
function CollectionOfficerDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const officerResponse = await axios.get(`officers/${user?.officerId || user?._id}`);
      ...
      console.log('👥 Assigned users from user_collections:', assignedUsers.length);
      ...
      const loanUsers = assignedUsers.filter(user => user.account_type === 'loan account').length;
      const savingUsers = assignedUsers.filter(user => user.account_type === 'saving account').length;
      ...
      console.log('📊 Focus on assigned users only - no collection data needed');
  ...
  const fetchPeriodData = async (period) => {
    try {
      ...
      const totalLoanAmount = loanCollections.reduce((sum, collection) => sum + (collection.amount || 0), 0);
      const totalSavingAmount = savingCollections.reduce((sum, collection) => sum + (collection.deposit_amount || 0), 0);
      const totalCollections = loanCollections.length + savingCollections.length;
      const totalAmount = totalLoanAmount + totalSavingAmount;
      
      // Get unique users
      const loanUsers = new Set(loanCollections.map(c => c.user_id?._id)).size;
      const savingUsers = new Set(savingCollections.map(c => c.user_id?._id)).size;
        ...
        >
          <div className="flex items-center justify-between mt-0">
            <div>
              ...
              <p className="text-sm sm:text-base text-gray-600">Collection Officer Dashboard - Track your collections and targets</p>
            </div>
         ...
         <motion.div 
           ...
           className="py-6"
         >
           <div className="grid grid-cols-5 gap-4 overflow-x-auto" style={{ minWidth: '1000px' }}>
             {/* Total Collection */}
             <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow min-w-[200px]">
               <div className="flex items-center justify-between">
                 <div>
                   <p className="text-sm font-medium text-gray-600">{selectedPeriod.charAt(0).toUpperCase() + selectedPeriod.slice(1)} Collections</p>
                   <p className="text-2xl font-bold text-green-600">{periodStats.totalCollections}</p>
                 </div>
             ...
             <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow min-w-[200px]">
               <div className="flex items-center justify-between">
                 <div>
                   ...
                   <p className="text-2xl font-bold text-blue-600">₹{periodStats.totalAmount.toLocaleString()}</p>
                 </div>
             ...
             <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow min-w-[200px]">
               <div className="flex items-center justify-between">
                 <div>
                   ...
                   <p className="text-2xl font-bold text-purple-600">₹{periodStats.totalLoan.toLocaleString()}</p>
                 </div>
             ...
             <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow min-w-[200px]">
               <div className="flex items-center justify-between">
                 <div>
                   <p className="text-sm font-medium text-gray-600">Saving Amount</p>
                   <p className="text-2xl font-bold text-orange-600">₹{periodStats.totalSaving.toLocaleString()}</p>
                 </div>
             ...
             <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow min-w-[200px]">
               <div className="flex items-center justify-between">
                 <div>
                   ...
                   <p className="text-2xl font-bold text-indigo-600">{periodStats.loanUsers + periodStats.savingUsers}</p>
                 </div>
                      ...
                      <tr key={user.user_id || index} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{user.phone_number}</td>
                          ...
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                            ...
                            {user.account_type === 'loan account' ? 'Loan' : 'Saving'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          ₹{user.emiAmount || 0}
